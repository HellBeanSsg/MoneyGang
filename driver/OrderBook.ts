export class OrderbookDriver {
    ...
    resetQuoteBook(): void {
        if (this.quoteBook.size > 40) {
            for (let i = 0; i < 10; ++i) {
                ...
                if (
                    Math.max(...this.quoteBook.get(parseFloat(key))!) <
                    this.emergenceQuoteThreshold
                ) {
    ...
    sortDictByKey(data: Map<number, any>): any {
        let sorted: any = Array.from(data.keys()).map((key) => [
            key,
            data.get(key),
        ]);
